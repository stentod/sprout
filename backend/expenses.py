...
def get_daily_spending_analytics():
    ...
    try:
        ...
        # Ultra-simple date calculation - no complex functions
        base_date = datetime.now(timezone.utc).date()
        target_date = base_date + timedelta(days=day_offset)
        start_date = target_date - timedelta(days=days - 1)
        end_date = target_date + timedelta(days=1)  # Include the end day
        
        logger.info(f"Simple date calculation: start={start_date}, end={end_date} for {days} days")
        
        # Ultra-simple direct query - no helper functions
        sql = '''
            SELECT 
                amount,
                description,
...
def get_category_breakdown_analytics():
    ...
    try:
        ...
        logger.info(f"Category analytics simple date calculation: start={start_date}, end={end_date} for {days} days")
        
        # Ultra-simple direct query with category information
        sql = '''
            SELECT
                e.amount,
                e.description,
...
@expenses_bp.route('/analytics/test-ultra-simple', methods=['GET'])
@require_auth
@handle_errors
def test_analytics_ultra_simple():
    """Ultra-simple test endpoint to verify basic functionality"""
    try:
        user_id = get_current_user_id()
        logger.info(f"Ultra-simple analytics test for user {user_id}")
        
        # Test basic database connection
        test_query = "SELECT COUNT(*) as count FROM expenses WHERE user_id = %s"
        result = run_query(test_query, (user_id,), fetch_one=True)
        
        # Test the exact same date calculation as the analytics endpoints
        from datetime import datetime, timedelta, timezone
        base_date = datetime.now(timezone.utc).date()
        target_date = base_date + timedelta(days=0)  # day_offset = 0
        start_date = target_date - timedelta(days=29)  # days = 30
        end_date = target_date + timedelta(days=1)
        
        # Test the exact same query as daily spending analytics
        expense_query = """
            SELECT 
                amount,
                description,
            ...
            LIMIT 5
        """
        expense_result = run_query(expense_query, (user_id, start_date.isoformat(), end_date.isoformat()))
        
        return jsonify({
            'success': True,
            'message': 'Ultra-simple analytics test passed',
            'user_id': user_id,
            'total_expenses': result['count'] if result else 0,
            'sample_expenses': expense_result,
            'date_range': {
